...
**Prequisites:**
* An OpenShift Cluster with **KServe Raw Deployment** configured
...
3. Deploy the TrustyAI Service configmap
    ```
    oc apply -f trustyai_cm.yaml -n $TEST_NS
    ```

4. Deploy the orchestrator configmap
    ```
    oc apply -f orchestrator_cm.yaml -n $TEST_NS
    ```

5. Deploy the vLLM gateway configmap
    ```
...
9. Query the `/pii` endpoint to perform completions while enabling detections on email and SSN information
    ```
    curl localhost:8090/pii \
...
10. Query the `/passthrough` endpoint to perform completions on while disabling detections
    ```
